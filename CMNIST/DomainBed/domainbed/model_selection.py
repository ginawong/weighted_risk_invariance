...
class OracleSelectionMethod(SelectionMethod):
    ...
    name = "test-domain validation set acc (oracle)"
    ...
    def run_acc(self, run_records):
        ...
        result = {
            ...
            'test_ece': chosen_record.get(test_in_ece_key, None)
        }
        return result

class OracleSelectionMethod2(SelectionMethod):
    """Like OracleSelectionMethod but also allows for early stopping, .i.e. take argmax
    across all checkpoints"""
    name = "test-domain early-stopping val (oracle2)"
    ...
    def run_acc(cls, run_records):
        ...
        test_in_acc_key = 'env{}_in_acc'.format(test_env)
        test_in_ece_key = 'env{}_in_ece'.format(test_env)
        chosen_record = run_records.argmax(lambda r: r[test_out_acc_key])
        return {
            'val_acc':  chosen_record[test_out_acc_key],
            'test_acc': chosen_record[test_in_acc_key],
            'test_ece': chosen_record.get(test_in_ece_key, None),
            'step': chosen_record['step'],
            'output_dir': chosen_record['args']['output_dir'],
            'test_env': test_env,
        }

class IIDAccuracySelectionMethod(SelectionMethod):
    """Picks argmax(mean(env_out_acc for env in train_envs))"""
    name = "training-domain validation set acc"
    ...
    def _step_acc(self, record):
        ...
        return {
            ...
            'test_ece': record.get(test_in_ece_key, None),
        }
...
class LeaveOneOutSelectionMethod(SelectionMethod):
    ...
    name = "leave-one-domain-out cross-validation acc"
    ...
    def _step_acc(self, records):
        ...
        return {
            ...
            'test_ece': test_records[0].get('env{}_in_ece'.format(test_env), None),
        }
