...
from domainbed.model_based_density.average_meter import AverageMeter
...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        if len(input_shape) > 1:
            ...
            feature_dims = self.featurizer.n_outputs
        else:
            self.featurizer = nn.Identity()
            feature_dims = input_shape[0]
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.update_count >= self.hparams['iters']:
            ...
            if self.hparams['class_balanced']:
                # for stability when facing unbalanced binary_labels across environments
...
class CAD(AbstractCAD):
    """Contrastive Adversarial Domain (CAD) bottleneck
       ...
       Properties:
       - Minimize I(D;Z)
       - Require access to domain binary_labels but not task binary_labels
       """
...
class CondCAD(AbstractCAD):
    ...
    Properties:
    - Minimize I(D;Z|Y)
    - Require access to both domain binary_labels and task binary_labels
    """
    def __init__(self, input_shape, num_classes, num_domains, hparams):
...
class _Generic_WRI(ERM):
    """Weighted Risk-Invariance algorithm"""
    def __init__(self, nll_type, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.nll_type = nll_type.lower()
        assert self.nll_type in ['erm']
        self.register_buffer('update_count', torch.tensor([0]))
        self.average_densities = None
        self.reset_density_stats()

        self.average_loss = AverageMeter(tau=500)
        self.average_nll = AverageMeter(tau=500)
        self.average_penalty = AverageMeter()

    def reset_density_stats(self):
        self.average_densities = defaultdict(lambda: defaultdict(lambda: AverageMeter(tau=5000)))

    def update(self, minibatches: List[Tuple[Tensor, Tensor]], density_models: Optional[OptimizedMultiDensityModel] = None):
        assert density_models is not None

        if self.update_count >= self.hparams['wri_penalty_anneal_iters']:
            penalty_weight = self.hparams['wri_lambda']
        else:
            penalty_weight = 0.0
        ...
        for env_idx, (x, y) in enumerate(minibatches):
            logits = self.network(x)
            if self.nll_type == 'erm':
                nll = F.cross_entropy(logits, y, reduction='none')
            else:
                raise NotImplementedError

            for density_env_idx in range(num_envs):
                with torch.no_grad():
        ...
        penalties = []
        for env1_idx in range(num_envs):
            for env2_idx in range(env1_idx + 1, num_envs):
        ...
        self.update_count += 1
        return {'loss': loss.item(), 'nll': nll.item(), 'penalty': (penalty_weight * penalty).item()}

    def predict(self, x, density=None, other_densities=None):
        if self.nll_type == 'erm' or density is None or other_densities is None:
            return super().predict(x)

        avg_density = torch.cat([density, other_densities], dim=1).mean(dim=1, keepdims=True)
        return self.network(x) * avg_density  # predict returns logits

    def penultimate(self, x):
        z = self.network[0](x)
        if hasattr(self.network[1], "penultimate"):
            z = self.network[1].penultimate(x)
        return z


class ERM_WRI(_Generic_WRI):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__("erm", input_shape, num_classes, num_domains, hparams)
