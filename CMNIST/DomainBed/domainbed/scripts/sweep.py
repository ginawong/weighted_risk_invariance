    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        for k, v in sorted(self.train_args.items()):
            if isinstance(v, list):
                ...
                if v in {'store_true', 'store_false'}:
                    command.append(f'--{k}')
                    continue
                else:
                    v = shlex.quote(v)
...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, density_method, cache_raw, featurizer_dims_override,
    hparams):
    args_list = []
            ...
            for algorithm in algorithms:
                ...
                for test_envs in all_test_envs:
                    for hparams_seed in range(n_hparams_from, n_hparams):
                        ...
                        train_args['autoencoder_arch'] = 'resnet50'
                        train_args['autoencoder_pretrained'] = 'store_true'
                        train_args['density_method'] = density_method
                        if featurizer_dims_override is not None:
                            train_args['featurizer_dims_override'] = featurizer_dims_override
                        train_args['pretrained_featurizer'] = 'store_true'
                        if cache_raw:
                            train_args['save_model_every_checkpoint'] = 'store_true'

                        if train_args["test_envs"][0] == 2:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cache_raw', action='store_true')
    ...
    args_list = make_args_list(
        ...
        featurizer_dims_override=args.featurizer_dims_override,
