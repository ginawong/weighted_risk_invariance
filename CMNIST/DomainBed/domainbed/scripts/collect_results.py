...
def format_mean(data, latex, scale=True):
    """Given a list of datapoints, return a string describing their mean and
    standard error"""
    if len(data) == 0:
        return None, None, "X"
    mean = (100 if scale else 1) * np.mean(list(data))
    err = (100 if scale else 1) * np.std(list(data) / np.sqrt(len(data)))
    if latex:
        return mean, err, (f"{{:.{3-2*scale}f}} $\\pm$ {{:.{3-2*scale}f}}").format(mean, err)
    else:
        return mean, err, (f"{{:.{3-2*scale}f}} +/- {{:.{3-2*scale}f}}").format(mean, err)

def print_table(table, header_text, row_labels, col_labels, colwidth=10,
    latex=True):
    """Pretty-print a 2D array of data, optionally with row/col binary_labels"""
    print("")
...
def print_results_tables(records, selection_method, latex):
    ...
    for dataset in dataset_names:
        ...
        for i, algorithm in enumerate(alg_names):
            ...
            for j, test_env in enumerate(test_envs):
                trial_groups = grouped_records\
                    .filter_equals(
                        "dataset, algorithm, test_env",
                        (dataset, algorithm, test_env)
                    )
                trial_accs = (trial_groups.select("sweep_ece" if args.ece else "sweep_acc"))
                mean, err, table[i][j] = format_mean(trial_accs, latex, scale=not args.ece)
                means.append(mean)
            if None in means:
                table[i][-1] = "X"
            else:
                table[i][-1] = ("{:.3f}" if args.ece else "{:.1f}").format(sum(means) / len(means))
        ...
        for j, dataset in enumerate(dataset_names):
            trial_averages = (grouped_records
                ...
                .map(lambda trial_seed, group:
                    group.select("sweep_ece" if args.ece else "sweep_acc").mean()
                )
            )
            mean, err, table[i][j] = format_mean(trial_averages, latex, scale=not args.ece)
            means.append(mean)
        if None in means:
            table[i][-1] = "X"
        else:
    ...
    print_table(table, header_text, alg_names, col_labels, colwidth=27, latex=latex)
...
if __name__ == "__main__":
    ...
    if args.ece:
        results_file = "results_ece.tex" if args.latex else "results_ece.txt"
    else:
        results_file = "results.tex" if args.latex else "results.txt"

    sys.stdout = misc.Tee(os.path.join(args.input_dir, results_file), "w")

    records = reporting.load_records(args.input_dir, args.algorithms, args.datasets)
    ...
    SELECTION_METHODS = [
        model_selection.OracleSelectionMethod2,
        # model_selection.ECESelectionMethod,
    ]
