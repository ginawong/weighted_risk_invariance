...
def _hparams(algorithm, density_method, pretrained_featurizer, dataset, random_seed):
    """
    ...
    elif algorithm in ["ERM_WRI"]:
        _hparam('wri_lambda', 500, lambda r: float(10 ** r.uniform(-1, np.log10(1000)))),
        _hparam('wri_penalty_anneal_iters', 0, lambda r: int(r.choice([0, 10])))

    if algorithm in ["ERM_WRI"]:
        _hparam("steps_per_density_update", 1, lambda r: int(r.choice([1, 2, 4, 8])))
        _hparam("mbd_lr", 0.04, lambda r: float(10 ** r.uniform(-2, np.log10(0.05))))
        ...
        _hparam("mbd_wri_lambda", 50, lambda r: float(10 ** r.uniform(np.log10(5), np.log10(50))))
        _hparam("mbd_ent_lambda", 0.1, lambda r: float(10 ** r.uniform(np.log10(0.05), np.log10(1))))
        _hparam("mbd_fit_steps", 4, lambda r: int(r.choice([1, 2, 4])))
        _hparam("mbd_min_density", 0.1, lambda r: float(r.uniform(0.01, 0.15)))
        _hparam("mbd_max_density", 1.0, lambda r: float(r.uniform(1.0, 2.0)))
...
def default_hparams(algorithm, density_method, pretrained_featurizer, dataset):
    return {a: b for a, (b, c) in _hparams(algorithm, density_method, pretrained_featurizer, dataset, 0).items()}


def random_hparams(algorithm, density_method, pretrained_featurizer, dataset, seed):
    return {a: c for a, (b, c) in _hparams(algorithm, density_method, pretrained_featurizer, dataset, seed).items()}
