...
from domainbed import classifiers
...
def train_featurizer(train_splits, input_shape, feature_dims, device, num_classes):
    assert all(weights is None for _, weights in train_splits)

    if input_shape[1:] == (28, 28):
        featurizer = autoencoders.train.train_mnist(train_splits, input_shape[0], feature_dims, device)
    elif input_shape == (3, 224, 224):
        # train classifier featurizer instead of autoencoder
        featurizer = classifiers.train.train(
            train_splits, feature_dims, num_classes, device,
            pretrained=args.autoencoder_pretrained,
            arch=args.autoencoder_arch,
            batch_size=args.autoencoder_batch_size,
            steps=args.featurizer_steps)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--featurizer_steps', type=int, default=10000)
    parser.add_argument('--autoencoder_pretrained', action='store_true')
    parser.add_argument('--autoencoder_arch', type=str, default='resnet50')
    ...
    if args.hparams_seed == 0:
        hparams = hparams_registry.default_hparams(args.algorithm, args.pretrained_featurizer, args.dataset)
    else:
        hparams = hparams_registry.random_hparams(args.algorithm, args.pretrained_featurizer, args.dataset,
    ...
    if args.pretrained_featurizer:
        ...
        featurizer = train_featurizer(train_in_splits, original_input_shape, hparams['pretrained_featurizer_dims'],
                                      device, num_classes=dataset.num_classes)

        print("Applying featurizer")
        in_splits = apply_featurizer(featurizer, in_splits, device)
        out_splits = apply_featurizer(featurizer, out_splits, device)
        uda_splits = apply_featurizer(featurizer, uda_splits, device)

    DENSITY_DEPENDENT_ALGS = ["WRI"]
    ...
    if args.algorithm in DENSITY_DEPENDENT_ALGS:
        ...
        assert args.pretrained_featurizer, "density estimators require the use of pretrained featurizer"
    ...
    train_loaders = [InfiniteDataLoader(
        ...
        num_workers=0 if args.pretrained_featurizer else dataset.N_WORKERS)
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                metrics = misc.accuracy(algorithm, loader, weights, device)
