...
DomainBed is a PyTorch suite containing benchmark datasets and algorithms for domain generalization, as introduced in [In Search of Lost Domain Generalization](https://arxiv.org/abs/2007.01434). The instructions here are copied from the [official DomainBed code repository](https://github.com/facebookresearch/DomainBed), with additional instructions on recreating the DomainBed results for WRI under [Recreating results with WRI](#recreating-results-with-wri).

This code starts from DomainBed commit `3ab2b63`, and was developed using python 3.8 and the requirements listed in `domainbed/requirements.txt`.

## Recreating results with WRI

Download VLCS, PACS, OfficeHome, TerraIncognita, and DomainNet into `domainbed_data` (following the [Quick start](#quick-start) instructions).

To run the full set of DomainBed experiments, run `sweep.sh` in the `domainbed/scripts` folder. This will take a long time. Adjust any experimental settings (remove datasets or methods) from the `sweep_config.sh` file in the `domainbed/scripts` folder. 

To collate the results run the `collect_results.sh` script in `domainbed/scripts`.
